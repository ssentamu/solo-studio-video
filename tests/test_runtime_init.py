...
class RuntimeInitStartupTests(unittest.TestCase):
    ...
    def test_startup_rejects_unverifiable_terminal_legacy_records(self):
        self.jobs_file.write_text(json.dumps(PRODUCTION_LEGACY_JOBS))

        first_boot = self.run_runtime_init()
        self.assertNotEqual(first_boot.returncode, 0)
        self.assertIn("InvalidStoreState", first_boot.stderr)
        self.assertEqual(job_store.list_jobs(path=self.database), [])
